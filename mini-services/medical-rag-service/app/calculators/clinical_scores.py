...
def calculate_cha2ds2_vasc(
    ...
    PROMPT 7 FIX: Female sex alone scores 0 (not 1) per Lip GYH et al. 
    Chest 2010;137:263-272 (PMID 20299623). Female sex is a risk modifier,
    not an independent risk factor when score is otherwise 0.
    ...
    - DM (1 point)
    - Stroke/TIA (2 points)
    - Vascular disease (1 point) - includes PAD, prior MI, aortic plaque
    - Age 65-74 (1 point)
    - Female (1 point) - ONLY if other risk factors present
    ...
    # PROMPT 7 FIX: Vascular disease (1 point)
    # Includes: PAD, prior MI, aortic plaque (per ESC 2020)
    if vascular_disease:
        score += 1
        components.append("Vascular disease: 1")
    
    # PROMPT 7 FIX: Female sex scoring
    # Female sex alone scores 0 - only adds point if other risk factors present
    # Reference: Lip GYH et al. Chest 2010;137:263-272 (PMID 20299623)
    is_female = sex.lower() in ["female", "f"]
    if is_female and score > 0:
        # Female sex adds 1 point only if other risk factors exist
        score += 1
        components.append("Female: 1")
    elif is_female:
        # Female sex alone - no point added, but note for interpretation
        components.append("Female: 0 (sex alone not a risk factor)")
    ...
    if is_male:
        ...
        # PROMPT 7 FIX: For females, score of 0 (female alone with no other factors) = low risk
        if score == 0:
    ...
    result = CalculatorResult(
        ...
        evidence="Lip GY, Nieuwlaat R, Pisters R, et al. Refining clinical risk stratification for predicting stroke and thromboembolism in atrial fibrillation using a novel risk factor-based approach. Chest 2010;137(2):263-272. PMID: 20299623",
    )
...
def calculate_has_bled(
    ...
    PROMPT 7 FIX: Added proper definition for "labile INRs" criterion.
    ...
    - L: Labile INR (1)
        PROMPT 7 FIX: Definition = TTR < 60% OR ≥3 INR values out of range 
        in last 12 measurements (Pisters R et al. Chest 2010, PMID 20299623)
    - E: Elderly (>65) (1)
    - D: Drugs/alcohol (1-2)
...
def calculate_curb65(
    ...
    bun: float = 0,  # BUN value
    bun_unit: str = "mg/dL",  # PROMPT 7: "mg/dL" (US) or "mmol/L" (SI)
    ...
    PROMPT 7 FIX: Added bun_unit parameter for US/SI unit conversion.
    Threshold: BUN > 19 mg/dL (US) OR Urea > 7 mmol/L (SI).
    ...
    score = 0
    components = []
    units_used = bun_unit  # PROMPT 7: Track units used
    
    # PROMPT 7 FIX: Convert BUN to mg/dL if needed
    bun_mg_dl = bun
    if bun_unit.lower() in ["mmol/l", "mmol"]:
        # Convert urea (mmol/L) to BUN (mg/dL)
        # BUN = Urea × 2.8
        bun_mg_dl = bun * 2.8
        units_used = f"{bun_unit} → {bun_mg_dl:.1f} mg/dL"
    ...
    # PROMPT 7 FIX: Urea threshold with proper unit handling
    # Threshold: BUN > 19 mg/dL (US) OR Urea > 7 mmol/L (SI)
    bun_threshold = 19 if bun_unit.lower() in ["mg/dl", "mg/dl"] else 7
    bun_elevated = bun_mg_dl > 19 if bun_unit.lower() in ["mg/dl", "mg/dl"] else bun > 7
    
    if bun_elevated:
        score += 1
        if bun_unit.lower() in ["mmol/l", "mmol"]:
            components.append(f"Urea>{bun}mmol/L (>7): 1")
        else:
            components.append(f"BUN>{bun:.0f}mg/dL (>19): 1")
    ...
    # PROMPT 7: Add units_used field
    return {**result.to_dict(), "components": components, "units_used": units_used}
