...
def _evaluate_penicillin_cephalosporin_conflict(
    ...
    # Rash (delayed hypersensitivity) - moderate risk for 1st/2nd gen
    if allergy_type == AllergyType.RASH:
        if generation == "FIRST_GEN":
            return AllergyConflictResult(
                blocked=False,
                severity=ConflictSeverity.CAUTION,
                warning=f"⚠️ CAUTION: ~2% cross-reactivity with first-generation cephalosporins. Consider skin test or alternative if available.",
                cross_reactivity_risk="~2%",
                alternative_recommendations=[
                    "Consider 3rd generation cephalosporin",
                    "Aztreonam (no cross-reactivity)",
                    "Perform penicillin skin testing if available",
                ],
                evidence_source="Macy E et al. JAMA Intern Med 2014;174:1630-1638",
            ...
            return AllergyConflictResult(
                blocked=False,
                severity=ConflictSeverity.CAUTION,
                warning=f"⚠️ CAUTION: ~1% cross-reactivity with second-generation cephalosporins. Monitor for reaction.",
            ...
            return AllergyConflictResult(
                blocked=False,
                severity=ConflictSeverity.SAFE,
                warning=f"NOTE: <1% cross-reactivity with {generation.replace('_', ' ').lower()} - generally safe to use. Risk equivalent to general population.",
                cross_reactivity_risk="<1%",
                alternative_recommendations=[],
                evidence_source="Macy E et al. JAMA Intern Med 2014; Romano A et al. JACI 2004",
                drug_class=f"Cephalosporin {generation}",
                allergy_detail="Penicillin rash",
            )
    
    # Anaphylaxis (IgE-mediated) - highest risk
    if allergy_type == AllergyType.ANAPHYLAXIS:
        if generation in ["FIRST_GEN", "SECOND_GEN"]:
            return AllergyConflictResult(
                blocked=True,
                severity=ConflictSeverity.CONTRAINDICATED,
                warning=f"⚠️ CONTRAINDICATED: History of penicillin anaphylaxis - avoid {generation.replace('_', ' ').lower()} cephalosporins due to shared side chains",
            ...
            return AllergyConflictResult(
                blocked=False,
                severity=ConflictSeverity.CAUTION,
                warning=f"⚠️ CAUTION: History of penicillin anaphylaxis. Cross-reactivity with {generation.replace('_', ' ').lower()} is <1% but use with caution and have emergency medications available.",
                cross_reactivity_risk="<1%",
                alternative_recommendations=[
                    "Consider Aztreonam (no cross-reactivity)",
