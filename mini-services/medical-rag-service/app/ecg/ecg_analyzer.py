...
class QTcFormulas:
    ...
    PROMPT 5 FIX: Enhanced with primary formula selection based on HR.
    ...
    1. Bazett: QTc = QT / sqrt(RR)
       ...
       - LIMITATION: Overcorrects at HR > 100, undercorrects at HR < 60
    
    2. Fridericia: QTc = QT / cbrt(RR) 
       - Most accurate across wide HR range (RECOMMENDED for most cases)
       - Reference: Fridericia LS. Acta Med Scand 1920;54:467-486
       - Preferred at HR > 100 per AHA/ACC 2013 (Rautaharju et al. PMID 19549168)
    
    3. Hodges: QTc = QT + 1.75 × (HR - 60)
       - Linear correction, best for extreme HR values
       - Reference: Hodges M. In: Noninvasive Electrocardiology 1992;61:405
       - PREFERRED at HR < 60 (bradycardia)
    
    4. Framingham: QTc = QT + 0.154 × (1 - RR) × 1000
       - Linear regression from Framingham cohort
       - Reference: Sagie A et al. Am J Cardiol 1992 (PMID 1632299)
    
    Primary QTc Selection Logic (PROMPT 5 FIX):
    - HR 60-100: use Fridericia (most accurate in normal range)
    - HR > 100: use Fridericia (Bazett overcorrects at tachycardia)
    - HR < 60: use Hodges (most accurate at bradycardia)
    """
    bazett: float
    fridericia: float
    hodges: float
    framingham: float
    primary: float  # Formula selected based on HR
    primary_formula: str = "Fridericia"  # Will be updated based on HR
...
class ECGInterval:
    ...
    # PROMPT 5 FIX: QRS validation fields
    qrs_validation: Optional[str] = None  # "normal", "narrow", "wide"
    jt_interval_ms: Optional[float] = None  # JT = QT - QRS (for wide QRS)
    wide_qrs_warning: Optional[str] = None  # Warning for wide QRS
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        # PROMPT 5 FIX: Add QRS validation fields
        if self.qrs_validation:
            result["qrs_validation"] = self.qrs_validation
        if self.jt_interval_ms is not None:
            result["jt_interval_ms"] = round(self.jt_interval_ms, 1)
        if self.wide_qrs_warning:
            result["wide_qrs_warning"] = self.wide_qrs_warning
        return result
...
class ECGAnalyzer:
    ...
    def _analyze_intervals(
        ...
        PROMPT 5 FIX: Enhanced with:
        - Primary formula selection based on heart rate
        - QRS validation (narrow/wide)
        - JT interval calculation for wide QRS
        ...
        # QRS validation variables
        qrs_validation = "normal"
        wide_qrs_warning = None
        jt_interval = None
        ...
        # QRS duration - PROMPT 5 FIX: Enhanced validation
        # Reference: Rautaharju PM et al. AHA/ACCF/HRS 2009
        if qrs is not None:
            # PROMPT 5 FIX: QRS validation
            # Normal: 70-120ms
            # Narrow (<70ms): Possible measurement error
            # Wide (>120ms): BBB, IVCD, hyperkalemia, pacing
            if qrs < 70:
                qrs_validation = "narrow"
                interp = f"⚠️ NARROW QRS ({qrs}ms) - Possible measurement error. Normal QRS is 70-120ms. Verify lead placement and measurement technique."
            elif qrs <= 120:
                qrs_validation = "normal"
                interp = "Normal QRS duration"
            elif qrs <= 140:
                qrs_validation = "wide"
                interp = "Wide QRS - incomplete bundle branch block or IVCD"
            else:
                qrs_validation = "wide"
                interp = "Wide QRS - consider complete BBB, hyperkalemia, pacing, or VT"
                wide_qrs_warning = (
                    f"⚠️ WIDE QRS ({qrs}ms): QTc measurement unreliable. "
                    f"Consider using JT interval (JT = QT - QRS). "
                    f"Reference: Rautaharju 2009 (PMID 19549168)"
                )
            ...
            intervals.append(ECGInterval(
                ...
                is_normal=is_normal,
                interpretation=interp,
                qrs_validation=qrs_validation,
                wide_qrs_warning=wide_qrs_warning,
            ))

        # QT interval - Calculate QTc using all four validated formulas
        # PROMPT 5 FIX: Enhanced with HR-based formula selection and JT interval
        # Reference: Rautaharju PM et al. AHA/ACCF/HRS 2009
        if qt is not None and hr > 0:
            ...
            # 2. Fridericia: QTc = QT / cbrt(RR)
            #    Most accurate across wide HR range
            ...
            # PROMPT 5 FIX: Primary formula selection based on HR
            # Reference: Rautaharju PM et al. AHA/ACCF/HRS 2009 Recommendations
            # - HR 60-100: Fridericia (most accurate in normal range)
            # - HR > 100: Fridericia (Bazett overcorrects at tachycardia)
            # - HR < 60: Hodges (most accurate at bradycardia)
            if hr < 60:
                primary_qtc = qtc_hodges
                primary_formula = "Hodges"
                formula_rationale = "Hodges selected (HR < 60, most accurate at bradycardia)"
            elif hr > 100:
                primary_qtc = qtc_fridericia
                primary_formula = "Fridericia"
                formula_rationale = "Fridericia selected (HR > 100, avoids Bazett overcorrection)"
            else:
                primary_qtc = qtc_fridericia
                primary_formula = "Fridericia"
                formula_rationale = "Fridericia selected (HR 60-100, most accurate in normal range)"
            ...
            # PROMPT 5 FIX: Calculate JT interval for wide QRS
            # Reference: Rautaharju PM. J Electrocardiol 2009;42(4):336-341 (PMID 19549168)
            # JT interval = QT - QRS (more accurate when QRS is prolonged)
            ...
            interp = self._generate_qtc_interpretation(
                primary_qtc, qtc_status, qtc_formulas, gender, formula_rationale, qrs, jt_interval
            )
    ...
    def _generate_qtc_interpretation(
        ...
        qtc_formulas: QTcFormulas,
        gender: Optional[str] = None,
        formula_rationale: Optional[str] = None,
        qrs: Optional[float] = None,
        jt_interval: Optional[float] = None,
    ) -> str:
        """Generate clinical interpretation for QTc interval.
        
        PROMPT 5 FIX: Enhanced with formula rationale and JT interval info.
        """
        gender_str = f" ({gender.lower()})" if gender else ""
        rationale_str = f" {formula_rationale}." if formula_rationale else ""
        jt_str = ""
        if jt_interval is not None:
            jt_str = f" JT interval (QT-QRS): {jt_interval:.0f}ms (preferred for wide QRS)."
        
        if qtc_status == QTcStatus.NORMAL:
            return f"Normal QTc{gender_str}: {primary_qtc:.0f}ms ({qtc_formulas.primary_formula}).{rationale_str}{jt_str}"
        
        elif qtc_status == QTcStatus.BORDERLINE:
            return f"Borderline QTc prolongation{gender_str}: {primary_qtc:.0f}ms ({qtc_formulas.primary_formula}). Monitor for drug effects.{rationale_str}{jt_str}"
        
        elif qtc_status == QTcStatus.PROLONGED:
            return (
                f"Prolonged QTc{gender_str}: {primary_qtc:.0f}ms ({qtc_formulas.primary_formula}). "
                f"Review medications (QT-prolonging drugs), electrolytes (K+, Mg++, Ca++). "
                f"Risk of arrhythmias.{rationale_str}{jt_str}"
            )
        
        elif qtc_status == QTcStatus.CRITICAL:
            return (
                f"⚠️ CRITICAL QTc prolongation: {primary_qtc:.0f}ms ({qtc_formulas.primary_formula})! "
                f"High risk of Torsades de Pointes. "
                f"Discontinue QT-prolonging drugs immediately. "
                f"Check electrolytes. Consider IV magnesium.{jt_str}"
            )
        
        return f"QTc: {primary_qtc:.0f}ms ({qtc_formulas.primary_formula}){jt_str}"
