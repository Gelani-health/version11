 ...
 * 
 * PROMPT Enhancement: Aggressive context compression for freeing up space
 * 
 * Features:
 ...
 * - Aggressive compression for long-running sessions
 * - Essential-only mode for critical context preservation
 * 
 * HIPAA Compliance:
 * - Preserves patient-related context
 * - Retains critical safety information
 * - Never purges audit-relevant data
 */
...
// Aggressive compression settings for space-constrained scenarios
export const AGGRESSIVE_OPTIONS: Required<CompressionOptions> = {
  maxMessages: 10,
  maxAgeMs: 10 * 60 * 1000, // 10 minutes
  preserveCritical: true,
  preservePatientContext: true,
  targetCompressionRatio: 0.3, // Reduce to 30% of original size
};

// Essential-only compression - keep only critical patient info
export const ESSENTIAL_ONLY_OPTIONS: Required<CompressionOptions> = {
  maxMessages: 5,
  maxAgeMs: 5 * 60 * 1000, // 5 minutes
  preserveCritical: true,
  preservePatientContext: true,
  targetCompressionRatio: 0.15, // Reduce to 15% of original size
};
...
// =============================================================================
// AGGRESSIVE COMPRESSION FOR SPACE OPTIMIZATION
// =============================================================================

/**
 * Aggressive context compression for space-constrained scenarios
 * Uses tighter limits and more aggressive summarization
 */
export function compressContextAggressive(
  messages: ContextMessage[],
): CompressionResult {
  return compressContext(messages, AGGRESSIVE_OPTIONS);
}

/**
 * Essential-only compression - keep only the most critical information
 * Use when context is severely constrained
 */
export function compressContextEssential(
  messages: ContextMessage[],
): CompressionResult {
  return compressContext(messages, ESSENTIAL_ONLY_OPTIONS);
}

/**
 * Extract only essential medical facts from a message
 * Returns a condensed version with critical information only
 */
export function extractEssentialFacts(message: ContextMessage): string {
  const content = message.content;
  const facts: string[] = [];
  ...
  const icdMatches = content.match(/[A-Z]\d{2}(\.\d+)?/g);
  if (icdMatches) {
    facts.push(`ICD: ${[...new Set(icdMatches)].join(', ')}`);
  }
  
  // Extract medication names (common patterns)
  const medPatterns = [
    /(?:prescribed|taking|medication)\s*:?\s*([A-Za-z]+(?:\s+[A-Za-z]+)?)/gi,
    /(?:drug|medication)\s+([A-Za-z]+)/gi,
  ];
  for (const pattern of medPatterns) {
    const matches = content.matchAll(pattern);
    for (const match of matches) {
      if (match[1] && match[1].length > 2) {
        facts.push(`Med: ${match[1]}`);
      }
    }
  }
  
  // Extract allergy mentions
  if (content.toLowerCase().includes('allergy') || content.toLowerCase().includes('allergic')) {
    const allergyMatch = content.match(/allerg(?:y|ic)\s*(?:to)?\s*:?\s*([A-Za-z]+)/i);
    if (allergyMatch) {
      facts.push(`Allergy: ${allergyMatch[1]}`);
    }
  }
  
  // Extract critical values
  const criticalPatterns = [
    { pattern: /BP[:\s]*(\d+\/\d+)/i, label: 'BP' },
    { pattern: /HR[:\s]*(\d+)/i, label: 'HR' },
    { pattern: /SpO2[:\s]*(\d+)%?/i, label: 'SpO2' },
    { pattern: /temp(?:erature)?[:\s]*(\d+\.?\d*)/i, label: 'Temp' },
    { pattern: /weight[:\s]*(\d+\.?\d*)\s*kg/i, label: 'Wt' },
  ];
  ...
  // Extract alert/warning keywords
  const alertKeywords = ['critical', 'urgent', 'alert', 'warning', 'severe', 'emergency'];
...
/**
 * Compress multiple messages into a single summary context
 * Maximum compression for context window optimization
 */
export function compressToSingleSummary(
  messages: ContextMessage[],
): string {
  ...
  for (const message of messages) {
    ...
    const facts = extractEssentialFacts(message);
    if (facts) {
      allFacts.push(facts);
    }
    
    // Extract diagnoses
    const icdMatches = message.content.match(/[A-Z]\d{2}(\.\d+)?/g);
    if (icdMatches) {
      icdMatches.forEach(d => diagnoses.add(d));
    }
    
    // Check for critical flags
    const lowerContent = message.content.toLowerCase();
    if (lowerContent.includes('critical') || lowerContent.includes('emergency')) {
      criticalFlags.push(message.content.slice(0, 100));
    }
  }
  
  // Build compact summary
  const parts: string[] = ['[COMPRESSED CONTEXT]'];
  
  if (diagnoses.size > 0) {
    parts.push(`Dx: ${[...diagnoses].slice(0, 5).join(', ')}`);
  }
  
  if (allFacts.length > 0) {
    parts.push(`Facts: ${allFacts.slice(0, 10).join('; ')}`);
  }
  
  if (criticalFlags.length > 0) {
    parts.push(`Critical: ${criticalFlags.length} alerts`);
  }
  
  parts.push(`Messages: ${messages.length} compressed`);
...
export function getCompressionStats(
  ...
  return {
    ...
    reductionPercent: originalTokens > 0 
      ...
      : 0,
    messageReduction: originalMessages.length - compressedMessages.length,
  };
}

/**
 * Purge stale context from memory
 * Removes messages older than threshold while preserving critical info
 */
export function purgeStaleContext(
  messages: ContextMessage[],
  maxAgeMs: number = 60 * 60 * 1000, // 1 hour
): {
  retained: ContextMessage[];
  purged: number;
  ...
  for (const message of messages) {
    ...
    const isCritical = message.importance === 'critical' || 
                      hasCriticalInfo(message.content);
    const hasPatient = message.patientId || hasPatientContext(message.content);
    
    // Keep if: recent, critical, or patient-related
    if (age < maxAgeMs || isCritical || hasPatient) {
      retained.push(message);
      if (isCritical) criticalPreserved++;
    } else {
      purged++;
    }
  }
  
  return { retained, purged, criticalPreserved };
}
