 ...
 * 
 * Security Model:
 * - GET requests: Public for health checks (used by UI indicator)
 * - POST/PUT/DELETE: Admin only (require authentication)
 * 
 * CORS: All methods support CORS for cross-origin requests
 */
...
/**
 * OPTIONS - CORS preflight handler (Public)
...
const DEFAULT_RAG_SERVICES = [
  {
    ...
    serviceType: 'rag',
    capabilities: JSON.stringify(['query', 'diagnose', 'pubmed-search', 'renal-dosing', 'clinical-calculators', 'bayesian-reasoning']),
    isActive: true,
    isDefault: true,
    priority: 10,
...
/**
 * GET - Retrieve RAG configurations (Public for health checks)
 * This endpoint is public to allow the RAG indicator to check service status
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (!serviceName && Array.isArray(services) && services.length === 0) {
    ...
    if (checkHealth && Array.isArray(services)) {
      services = await Promise.all(
        services.map(async (service) => {
          // If already connected, skip health check
    ...
    // Return with CORS headers for cross-origin support
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to fetch RAG configuration', data: [] },
...
/**
 * POST - Create or Update RAG configuration (Admin only)
 */
...
/**
 * PUT - Set default RAG service (Admin only)
 */
...
/**
 * DELETE - Remove RAG configuration (Admin only)
 */
...
async function initializeDefaultServices() {
  console.log('[RAG] Initializing default RAG services...');
  for (const service of DEFAULT_RAG_SERVICES) {
    try {
      await db.rAGServiceConfig.create({ data: service as any });
      console.log(`[RAG] Created service: ${service.serviceName}`);
    } catch (error: any) {
      if (error.code === 'P2002') {
        console.log(`[RAG] Service already exists: ${service.serviceName}`);
      } else {
        console.error(`[RAG] Failed to create service ${service.serviceName}:`, error);
      }
    }
  }
}
