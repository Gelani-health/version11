 ...
 * Returns detailed breakdown by data type
 */
export async function estimatePurgeSavings(): Promise<{
  ...
  const estimates: { dataType: string; estimatedRecords: number; estimatedSizeKB: number; description: string }[] = [];

  // AI Interactions older than 90 days (non-patient)
  ...
  estimates.push({
    ...
    description: 'AI interactions without patient association, >90 days old',
  });
  ...
  estimates.push({
    ...
    description: 'RAG query logs >30 days old',
  });
  ...
  estimates.push({
    ...
    description: 'Draft voice notes >7 days old',
  });

  // Embedding cache older than 90 days or stale
  const staleDate = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
  const embedOld = await db.embeddingCache.count({
    where: {
      OR: [
  ...
  estimates.push({
    dataType: 'embeddingCache',
    estimatedRecords: embedOld,
    estimatedSizeKB: embedOld * 8, // ~8KB per embedding
    description: 'Embedding cache >90 days old or not accessed in 30 days',
  });

  // Integration sync logs older than 90 days
  const syncOld = await db.integrationSyncLog.count({
    where: { startedAt: { lt: cutoff90 } },
  });
  estimates.push({
    dataType: 'integrationSyncLogs',
    estimatedRecords: syncOld,
    estimatedSizeKB: syncOld * 1,
    description: 'Integration sync logs >90 days old',
  });
