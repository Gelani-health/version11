...
/**
 * Helper to get decrypted API key for testing
 */
  ...
  try {
    ...
    try {
      ...
      if (integration.provider === "zai") {
        ...
        const apiKey = getDecryptedApiKey(integration.apiKey);
        ...
        try {
          ...
          if (fetchError instanceof Error) {
            if (fetchError.message.includes('fetch failed') || fetchError.message.includes('ECONNREFUSED') || fetchError.name === 'AbortError') {
