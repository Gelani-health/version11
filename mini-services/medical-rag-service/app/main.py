...
class HybridQueryRequest(BaseModel):
    ...
    enable_bm25: bool = Field(True, description="Enable BM25 keyword search")
...
# Add hybrid_engine to AppState
AppState.hybrid_engine = None


@app.post("/api/v1/hybrid-query", response_model=HybridQueryResponse, tags=["P1 - Hybrid Retrieval"])
async def hybrid_retrieval_query(
    request: HybridQueryRequest,
    authenticated: bool = Depends(verify_api_key),
):
    """
    P1: Perform hybrid retrieval combining BM25 + Semantic search.
    ...
    This endpoint provides state-of-the-art retrieval for clinical decision support.
    """
    start_time = time.time()
    
    try:
        from app.retrieval.hybrid_retrieval import get_hybrid_retrieval_engine
        
        # Initialize hybrid engine
        if state.hybrid_engine is None:
            state.hybrid_engine = await get_hybrid_retrieval_engine()
        
        # Configure engine
        state.hybrid_engine.enable_bm25 = request.enable_bm25
        state.hybrid_engine.enable_expansion = request.enable_expansion
        state.hybrid_engine.min_score = request.min_score
        
        # Perform hybrid retrieval
        context = await state.hybrid_engine.retrieve(
            query=request.query,
            patient_context=request.patient_context,
            specialty=request.specialty,
            top_k=request.top_k,
        ...
        for result in context.results:
            formatted_results.append(HybridSearchResult(
                ...
                mesh_terms=result.mesh_terms[:5],
                doi=result.doi,
        ...
        return HybridQueryResponse(
            ...
            results=formatted_results,
            total_results=len(formatted_results),
            total_semantic_results=context.total_semantic_results,
            total_bm25_results=context.total_bm25_results,
            fusion_latency_ms=context.fusion_latency_ms,
            retrieval_latency_ms=context.retrieval_latency_ms,
...
async def get_hybrid_retrieval_stats():
    """P1: Get hybrid retrieval engine statistics."""
    if state.hybrid_engine:
        return state.hybrid_engine.get_stats()
    return {"status": "not_initialized"}


@app.post("/api/v1/hybrid/index-document", tags=["P1 - Hybrid Retrieval"])
async def index_document_to_bm25(
    doc_id: str,
    title: str,
    abstract: str,
    ...
    """
    P1: Index a document to the BM25 index.
    
    This should be called when documents are ingested into Pinecone
    to keep the BM25 index in sync.
    """
    from app.retrieval.hybrid_retrieval import get_hybrid_retrieval_engine
    
    try:
        engine = await get_hybrid_retrieval_engine()
        ...
        engine.index_document(doc_id, title, abstract, metadata)
        
        return {
            "status": "success",
            "doc_id": doc_id,
            "bm25_index_stats": engine.bm25_index.get_stats(),
        }
...
async def remove_document_from_bm25(
    ...
    try:
        ...
        logger.error(f"BM25 removal error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def sync_bm25_from_pinecone(
    ...
    """
    P1: Sync BM25 index from existing Pinecone vectors.
    
    This populates the BM25 index with documents already stored in Pinecone.
    Should be run once after deployment or when BM25 index is cleared.
    """
    from app.retrieval.hybrid_retrieval import get_hybrid_retrieval_engine
    ...
    try:
        ...
        # Note: Pinecone doesn't support "get all", so we use a dummy query
        dummy_vector = [0.0] * 768
        
        loop = asyncio.get_event_loop()
        response = await loop.run_in_executor(
            None,
            lambda: engine._index.query(
                vector=dummy_vector,
                top_k=max_vectors,
                namespace=settings.PINECONE_NAMESPACE,
        ...
        return {
            "status": "success",
            "vectors_scanned": len(response.matches),
...
async def clear_bm25_index(
    ...
    """P1: Clear the BM25 index."""
    from app.retrieval.hybrid_retrieval import get_hybrid_retrieval_engine
    
    try:
        engine = await get_hybrid_retrieval_engine()
        engine.clear_bm25_index()
        ...
        logger.error(f"BM25 clear error: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/api/v1/recency-score", tags=["P1 - Hybrid Retrieval"])
async def calculate_recency_score_endpoint(
    publication_date: str,
    domain: Optional[str] = None,
):
    """
    P1: Calculate recency score for a publication date.
    ...
    - anatomy: Slow (anatomical knowledge stable)
    """
    from app.retrieval.hybrid_retrieval import RecencyScorer
    
    scorer = RecencyScorer()
    recency_score = scorer.get_recency_score(publication_date)
    weighted_score = scorer.calculate_score(publication_date, 1.0, domain)
    ...
    return {
        ...
        "decay_years": scorer.decay_years,
    }
